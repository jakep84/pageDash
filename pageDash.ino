...
void setup()
{
  //Double Blink to display program running 
  pinMode(led,OUTPUT);
  ...
  typeln("open preamble55.html");
  //go back to the terminal 
  cmd(KEY_TAB);
  //remove the file you created
  typeln("rm -rf preamble5.html");
  //close the terminal 
  cmd(KEY_Q);
  //Turn on LED to display process complete
  digitalWrite(led, HIGH);
}
